...
class GrowlRegistrationPacket:
	...
	def payload(self):
		...
		print "+Header(ver,type,len)" + binascii.b2a_hex(self.data)
		self.data += struct.pack( "BB",
								  len(self.notifications),
								  len(self.defaults) )
		print "+Notification+Defaults" + binascii.hexlify(self.data)
		self.data += self.application
		print "+ApplicationName" + binascii.hexlify(self.data)
		for notification in self.notifications:
			encoded = notification.encode("utf-8")
			self.data += struct.pack("!H", len(encoded))
			self.data += encoded
		print "+Notifications" + binascii.hexlify(self.data)
		for default in self.defaults:
			self.data += struct.pack("B", default)
		print "+Defaults" + binascii.hexlify(self.data)
		self.checksum = md5.new()
		self.checksum.update(self.data)
		print "CheckSum=" + binascii.b2a_hex(self.checksum.digest())
		if self.password:
			self.checksum.update(self.password)
		self.data += self.checksum.digest()
		print "+CheckSumDigest" + binascii.hexlify(self.data)
		return self.data
	# end def
# end class
